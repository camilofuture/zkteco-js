...
class ZTCP {
	...
	async getAttendances(callbackInProcess = () => {}) {
		try {
			...
			// Enviar comando para obtener registros de asistencia
			const data = await this.readWithBuffer(REQUEST_DATA.GET_ATTENDANCE_LOGS, callbackInProcess);

			// Liberar datos del buffer del dispositivo una vez que se obtienen los registros
			if (this.socket) {
				await this.freeData(); // CMD_FREE_DATA (1502)
			}
			...
			// Debug: Mostrar los bytes crudos del buffer en hexadecimal
			console.log("Record data buffer - FULL HEX (BEFORE):", data.data.toString("hex"));

			// Process the record data
			let recordData = data.data.subarray(4); // Skip header

			// Debug: Mostrar el tamaño del buffer y los primeros bytes en decimal para más claridad
			console.log("Buffer length:", recordData.length);
			console.log("Record data buffer - HEX (AFTER HEADER):", recordData.toString("hex"));
			console.log("Record data buffer (FIRST 20 BYTES IN DECIMAL):", recordData.slice(0, 20));

			const records = [];

			// Process each attendance record
			while (recordData.length >= RECORD_PACKET_SIZE) {
				const currentRecord = recordData.subarray(0, RECORD_PACKET_SIZE);

				if (currentRecord.length !== RECORD_PACKET_SIZE) {
					console.warn("Tamaño de registro incorrecto:", currentRecord.length);
					break; // Salir del bucle si el tamaño es incorrecto
				}

				// Debug: Mostrar los datos crudos de cada registro antes de decodificar
				console.log("Raw record data:", currentRecord.toString("hex"));

				const record = decodeRecordData40(currentRecord);

				// Debug: Mostrar el registro decodificado
				console.log("Decoded record:", record);

				// Validar registros con user_id inválidos o fechas incorrectas
				if (record.user_id === "" || record.record_time.includes("2000")) {
					console.warn("Registro inválido encontrado:", record);
					recordData = recordData.subarray(RECORD_PACKET_SIZE); // Saltar al siguiente registro
					continue; // Ignorar el registro actual
				}

				// Agregar la IP del dispositivo a los registros para más claridad en la depuración
				records.push({ ...record, ip: this.ip });

				// Avanzar al siguiente paquete de datos
				recordData = recordData.subarray(RECORD_PACKET_SIZE);
			}

			// Retornar la lista de registros de asistencia
			return { data: records };
		} catch (err) {
			console.error("Error getting attendance records:", err);
			throw err; // Re-lanzar el error para manejarlo en el caller
		}
	}
